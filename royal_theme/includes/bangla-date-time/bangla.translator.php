...
if($rtbn_options['trans_dt'] == "1") {
    add_filter('date_i18n', 'en_to_bn');
}

if ( $rtbn_options['trans_dt'] == "2" ) {
...
if ( $rtbn_options['trans_cmnt'] == "1" ) {
    add_filter( 'comments_number', 'en_to_bn' );
    add_filter( 'get_comment_count', 'en_to_bn' );
}

if($rtbn_options['trans_num'] == "1") {
    add_filter('number_format_i18n', 'en_to_bn');
}

if($rtbn_options['trans_cal'] == "1") {

add_filter( 'get_archives_link', 'rtbn_en_to_bangla' );
add_filter( 'wp_list_categories', 'rtbn_en_to_bangla' );
add_filter( 'get_calendar' , 'rtbn_get_calendar_filter' , 10 , 2 );

function rtbn_get_calendar($initial = false, $echo = false) {
...
function rtbn_get_calendar_filter( $content ) {
  $output = rtbn_get_calendar();
  echo rtbn_en_to_bangla($output);
}
...
function rtbn_en_to_bangla($content=''){
